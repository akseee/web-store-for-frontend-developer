...
- `getTotal` - для установки итоговой цены за товары
...
|:----------:|----------|----------|
| `cards:change` | | |
| `card:preview` | изменение открываемой в модальном окне карточки| |
| `modal:open` | | Блокируем прокрутку страницы при открытии модального окна |
| `modal:close` | | Разблокируем прокрутку страницы при закрытии модального окна |
| `basket:open` | | |
| `basket:change` | изменения в корзине, очистка корзины | |
| `order:open` | | |
| `order:submit` | | |
| `order:change` | очистка заказа, | |
| `formErrors:change` | | |
